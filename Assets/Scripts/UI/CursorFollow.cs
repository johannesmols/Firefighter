...
public class CursorFollow : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if(Input.mousePosition.y+tooltip.rect.height > Screen.height ){
            vertical = -1;
        }else{
            vertical = 1;
        }
        
        transform.position = Input.mousePosition + new Vector3(horizontal*(tooltip.rect.width/2), vertical*(tooltip.rect.height/2), 0 );
    }
}
